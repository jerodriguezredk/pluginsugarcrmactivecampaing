...
class ImportLeads
{
    ...
    function __construct($args)
    {
        ...
        if ($validate_license === true) {
            $this->get_lead_beans($args);
        }
    }

    public function import_to_ac($api_creds, $ac_list_id, $crm_tl_id, $crm_tl_name)
    {
        $primary_email = $this->lead_bean->emailAddress->getPrimaryAddress($this->lead_bean);
        if ($primary_email) {
            $lead_data = $this->get_leads_data($ac_list_id, $primary_email);
            if ($lead_data) {
                $data_to_import = json_encode($lead_data);
                try {
                    ...
                    if (isset($output["result_code"]) && (int) $output["result_code"]) {
                        $this->save_stats($ac_list_id, $output["subscriber_id"], $crm_tl_name, $crm_tl_id, $this->lead_bean->id, $this->lead_bean->module_dir);
                        $this->lead_bean->active_campaign_id_c = $output["subscriber_id"];
                        $this->lead_bean->save();
                    } else {
                        $GLOBALS['log']->fatal("Unable to import lead in active campaign. " . basename(__FILE__) . ":" . __LINE__);
                        $GLOBALS['log']->fatal($output["error"] ?? "Something went wrong while syncing active campaign leads. " . basename(__FILE__) . ":" . __LINE__);
                ...
                $GLOBALS['log']->fatal("lead data not found for active campaign. " . basename(__FILE__) . ":" . __LINE__);
            }
        } else {
            $GLOBALS['log']->fatal("Email of lead is not fond. " . basename(__FILE__) . ":" . __LINE__);
        }
        return true;
    }

    private function get_leads_data($ac_list_id, $primary_email)
    {
        $lead_data = array();
        $custom_fileds = array();

        $f_mapped = $this->get_leads_fields_mappings();
        if ($f_mapped) {
            ...
            foreach ($field_mapping_array as $key => $info) {
                if ($key == "email") {
                    ...
                    $lead_data += array(
                        $key => $this->lead_bean->last_name
                    );
                } elseif ($key == "first_name") {
                    $lead_data += array(
                        $key => $this->lead_bean->first_name
                    );
                } elseif ($key == "phone") {
                    $lead_data += array(
                        $key => $this->lead_bean->phone_mobile
                    );
                } else {
                    if ($key != AcConstants::CRM_SOURCE && $key != AcConstants::CRM_RECORD_ID) {
                        $lead_data += array(
                            "field[%$key%,0]" => $this->lead_bean->{$info},
                        );
                    }
                }
            }
            $lead_data += array(
                        "field[%" . AcConstants::CRM_SOURCE . "%,0]" => $this->lead_bean->module_dir,
                        "field[%" . AcConstants::CRM_RECORD_ID . "%,0]" => $this->lead_bean->id,
                    );
            ...
            $GLOBALS['log']->fatal("leads fields mapping with active field mapping not found. " . basename(__FILE__) . ":" . __LINE__);
        }
        return $lead_data;
    }


    private function get_lead_beans($args)
    {
        $obj = new Lead();
    ...
    private function get_leads_fields_mappings()
    {
        $val = "";
